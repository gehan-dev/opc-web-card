...
```yaml
ecs_config:
  instance_type: ecs.e-c1m1.large    # 2 vCPU 2 GiB — 必须用 e 系列，禁止使用其他系列（u1/c7/g7 等）
  system_disk:
...
| 5 | **创建前必须说明！** 执行任何创建资源的命令前，先用自然语言告诉用户"接下来我要创建什么、大致配置、预估费用"，等用户确认后再执行 | 🔴 CRITICAL |
| 6 | **严格使用指定实例规格 ecs.e-c1m1.large！** 禁止替换为 u1/c7/g7 或任何其他系列，即使 AI 认为"性价比更高"也不可以 | 🔴 CRITICAL |
| 7 | **余额检查 ≥ 100 元** — 按量付费开通前提 | HIGH |
| 8 | **转包年前实际询价** — 不可硬编码价格，必须 DescribePrice 确认 | HIGH |
| 9 | **跑腿员比喻** — RAM 凭证解释必须用"跑腿员穿制服"比喻 | MEDIUM |
| 10 | **域名&备案非必要** — 今天只做简洁名片（公网IP），域名可提及但不强制 | MEDIUM |
| 11 | **活动限定** — 明确告知用户本 skill 仅适用于 2026/6/28 下午上海专场 | LOW |
